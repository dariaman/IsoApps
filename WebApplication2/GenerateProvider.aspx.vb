...
Public Class GenerateProvider
            ...
            If Not Page.IsPostBack Then
                If UserLogin.IsActive Then
                    ...
                    Catch ex As Exception
                        ...
                        Dim msg As String = String.Format("{0} - GenerateProvider - " & UserLogin.UserId & " - {1} - {2}{3}", Now.ToString("dd/MM/yyyy HH:mm:ss"), UserLogin.UserId, ex, Environment.NewLine)
                        WriteFile.Write(config.SetFullFilePath, msg)
                    End Try
                Else
                    Response.Redirect("login.aspx?p=GenerateProvider.aspx", False)
                End If
            End If
        Else
            Response.Redirect("login.aspx?p=GenerateProvider.aspx", False)
    ...
    Protected Sub bindData()
        Try
            Dim dt As DataTable = _Clsuser.SelectAllUser()
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        Try
            ...
            Session("flnm") = "Provider_" & Now.ToString("yyyyMMddHHmmss") & ".txt"
            ...
            objWriter.WriteLine("User Id | User Id 2 | User Name |  Password ")
            Do While i <= dt.Rows.Count - 1
                If dt.Rows(i)(4).ToString = "" Then

                    _Clsuser.UserIdInp = ""
                    _Clsuser.UserNameInp = Replace(dt.Rows(i)(3).ToString, "'", "`")
                    _Clsuser.EmailInp = ""

                    _Clsuser.RoleCodeInp = "00004"

                    _Clsuser.IsActiveInp = "True"
                    _Clsuser.UserId = UserLogin.UserId
                    _Clsuser.Mkt_Code = ""
                    _Clsuser.Branch = "00"
                    _Clsuser.PassGenUsr = dt.Rows(i)(2).ToString
                    '_Clsuser.BirthDate = DBNull.Value
                    '_Clsuser.Gender = DBNull.Value
                    _Clsuser.ExpirateDate = FormatDateTime("12/31/2030", DateFormat.ShortDate)
                    _Clsuser.Pict = ""
                    _Clsuser.providerid = dt.Rows(i)(0).ToString
                    _Clsuser.UserId2 = dt.Rows(i)(1).ToString
                    _Clsuser.UserIdInp = _Clsuser.InsertUserNewGENUSRPROV()
                    If _Clsuser.UserIdInp <> "" Then
                        _Clsuser.InsertUserBranch("00")
                        _Clsuser.InsertMSLOGINDETAIL(Trim(_Clsuser.UserIdInp), "", "", _Clsuser.providerid, _Clsuser.RoleCodeInp)
                        If System.IO.File.Exists(strFile) = True Then

                            objWriter.WriteLine(Trim(_Clsuser.UserIdInp) & " | " & Trim(_Clsuser.UserId2) & " | " & _Clsuser.UserNameInp & " | " & _Clsuser.PassGenUsr & "")
                ...
                Else
                    If System.IO.File.Exists(strFile) = True Then
                        ...
                        objWriter.WriteLine(Trim(dt.Rows(i)(1).ToString) & " | " & dt.Rows(i)(4).ToString & " | " & dt.Rows(i)(3).ToString & " | " & _ClsEncryption.Decrypt(dt.Rows(i)(2).ToString))
