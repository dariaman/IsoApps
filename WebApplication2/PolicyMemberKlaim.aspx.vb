...
Imports SPGeneral
Public Class PolicyMemberKlaim
        ...
        If Not UserLogin Is Nothing Then
            If Not Page.IsPostBack Then
                If UserLogin.IsActive Then
                    Try
                        Session("DashBoard") = "Klaim <i class='fa fa-file-text fa-fw'></i>"
                    ...
                    Catch ex As Exception
                        ...
                        Dim msg As String = String.Format("{0} - PolicyMemberKlaim - " & UserLogin.UserId & " - {1} - {2}{3}", Now.ToString("dd/MM/yyyy HH:mm:ss"), UserLogin.BranchCode, ex, Environment.NewLine)
                        WriteFile.Write(config.SetFullFilePath, msg)
                    End Try
                Else
                    Response.Redirect("login.aspx?p=PolicyMemberKlaim.aspx", False)
                End If
            End If
        Else
            Response.Redirect("login.aspx?p=PolicyMemberKlaim.aspx", False)
    ...
    Protected Sub bindData(type As String, KeyWord As String)
        Try
            GVVw_Claim_Info_Header.DataSource = _ClsPolicyMember.bindDataClaim_Info_Header_value(type, KeyWord)
            GVVw_Claim_Info_Header.DataBind()
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    ...
    Protected Sub bindDatapolicyno(type As String, value As String, policyno As String)
        Try
            GVVw_Claim_Info_Header.DataSource = _ClsPolicyMember.bindDataClaim_Info_Header_Policyno(type, value, policyno)
            GVVw_Claim_Info_Header.DataBind()
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    ...
    'Protected Sub bindDataklaim(policyno As String, KeyWord As String)
    '    Try
    '        GVVw_Claim_Info_Header.DataSource = _ClsPolicyMember.bindDataKlaim(type)
    '        GVVw_Claim_Info_Header.DataBind()
    '    Catch ex As Exception
    '        Throw New Exception(ex.Message)
    '    End Try
