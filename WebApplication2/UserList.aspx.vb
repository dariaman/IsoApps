...
Public Class Userlist
        ...
        If Not UserLogin Is Nothing Then
            If Not Page.IsPostBack Then
                If UserLogin.IsActive Then
                    Try
                        Session("DashBoard") = "Who is?<i class='fa fa-user fa-fw'></i>"
                        ...
                        End If
                        _sama.isiddlRole(ddlRoleAP, Session("RoleCode"))
                    ...
                    Catch ex As Exception
                        ...
                        Dim msg As String = String.Format("{0} - UserList - " & UserLogin.UserId & " - {1} - {2}{3}", Now.ToString("dd/MM/yyyy HH:mm:ss"), UserLogin.BranchCode, ex, Environment.NewLine)
    ...
    Protected Sub bindData(rolecode As String, Branchcode As String)
        Try
            Dim dt As DataTable
            dt = _clsrole.bindData(rolecode, txtKeyword.Text, Branchcode)
    ...
    Protected Sub LoadDataUser(ByVal userId As String)
        Try
            ...
            txtEmail.Text = dr.Rows(0).Item("Email").ToString()
            txtBirthDate.Text = Format(CDate(IIf(dr.Rows(0).Item("BirthDate").ToString() = "", Today, dr.Rows(0).Item("BirthDate").ToString())), "MM/dd/yyyy")
            txtExpirateDate.Text = Format(CDate(IIf(dr.Rows(0).Item("ExpirateDate").ToString() = "", Today, dr.Rows(0).Item("ExpirateDate").ToString())), "MM/dd/yyyy")
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click, LinkSubmit.Click
        Try
            ...
            If validasi() = True Then
                ...
                'If _Clsusers.DeleteProductAccess() = True Then
                '    For Each dgcheck As DataGridItem In gridMenu1.Items
                '        Dim chk As Boolean = CType(dgcheck.Cells(1).Controls(1), HtmlInputCheckBox).Checked
                '        If chk Then
                '            _Clsusers.TypeProduct = dgcheck.Cells(0).Text
                '            _Clsusers.InsertUserProduct(_Clsusers.TypeProduct)
                '        End If
                '    Next
                'Else
                '    ClientScript.RegisterStartupScript(Me.GetType, "confirm", "<script language=javascript>jqxAlert.Information('Data not saved!');</script>")
    ...
    Protected Sub ddlRoleAP_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlRoleAP.SelectedIndexChanged
        System.Threading.Thread.Sleep(500)
